...
修复后的真实补测已达到共同窗口：4015 个新增可见字符、104 对画面、2 次实际选择，3020 个证据文件及指标重算通过。两个新增章节都使用新的投影标记，原始正文与脚本/图坐标核对通过；这两章没有 HTML 标签，特定 HTML 缺陷仍由上面三份旧真实失败响应回放验证。其他两个项目的结果与采集限制见 [本轮并行重测](PARALLEL_RECHECK_20260907.md)。
