...
> **适用版本：`d16bdea`，CAMPUS-01-V3。**本页记录取消适配器预算上限后的首次文本重测，输出仍止于首个未执行选择；不代表现行图文窗口或后来 IF Line HTML 补丁的验证。无限预算政策继续适用，当前配置与批量流程请看[操作者交接说明](OPERATOR_HANDOFF.md)及[批量运行](BATCH_RUNNING.md)。
