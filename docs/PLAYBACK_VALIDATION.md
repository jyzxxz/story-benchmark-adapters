...
> **验证版本：单篇回放 `bd8b9f8`，整批自动交付 `ee1cf0d`。**本页记录离线导出与浏览器测试，不是接收者的操作步骤，也不证明新生成故事一定成功。生产与实验交接见[操作者交接说明](OPERATOR_HANDOFF.md)，本地打开与发给评审者见[回放使用说明](PLAYBACK_REVIEW.md)；后续代码变更需要相应的新验证，不能沿用本页计数冒充重测。
